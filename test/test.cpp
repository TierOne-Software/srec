...
TEST_CASE( "ASCIIToHexString", "[ASCIIToHexString]" ) {
    ...
    REQUIRE(ASCIIToHexString("").empty());
...
TEST_CASE("Srec base class functionality", "[Srec]") {
    SECTION("getTypeChar returns correct values") {
        Srec0 rec0(std::vector<uint8_t>{}); // Empty vector for header
        REQUIRE(rec0.getTypeChar() == '0');

        Srec1 rec1(0, std::vector<uint8_t>{}); // Empty vector for data
        REQUIRE(rec1.getTypeChar() == '1');

        Srec2 rec2(0, std::vector<uint8_t>{}); // Empty vector for data
        REQUIRE(rec2.getTypeChar() == '2');

        Srec3 rec3(0, std::vector<uint8_t>{}); // Empty vector for data
...
TEST_CASE("Srec5 record count (16-bit)", "[Srec5]") {
    ...
    SECTION("Maximum value check") {
        ...
        REQUIRE_NOTHROW(Srec5(0xFFFF));
        
        // Test with value exceeding 16-bit range
        REQUIRE_THROWS_AS(Srec5(0x10000), std::invalid_argument);
    }
}
...
TEST_CASE("Srec6 record count (24-bit)", "[Srec6]") {
    ...
    SECTION("Maximum value check") {
        // Test with valid value
        REQUIRE_NOTHROW(Srec6(0xFFFFFF));
        
        // Test with value exceeding 24-bit range
        REQUIRE_THROWS_AS(Srec6(0x1000000), std::invalid_argument);
    }
}
...
TEST_CASE("SrecFile operations", "[SrecFile]") {
    ...
    SECTION("Constructor and basic operations") {
        SrecFile sf(filename, SrecFile::AddressSize::BITS32);
        REQUIRE(sf.is_open());
        REQUIRE(sf.getFilename() == filename);
        REQUIRE(sf.addrsize() == SrecFile::AddressSize::BITS32);
        sf.close();
    }
    ...
    SECTION("write_record_payload with 24-bit address") {
        SrecFile sf(filename, SrecFile::AddressSize::BITS24);
        std::vector<uint8_t> data{0x01, 0x02, 0x03, 0x04};
        sf.write_record_payload(data);
        sf.close();
    ...
    SECTION("write_record_payload with 16-bit address") {
        SrecFile sf(filename, SrecFile::AddressSize::BITS16);
        std::vector<uint8_t> data{0x01, 0x02, 0x03, 0x04};
        sf.write_record_payload(data);
        sf.close();
...
TEST_CASE("CRC32 calculation", "[CRC32]") {
    SECTION("Empty data") {
        unsigned char data[] = {};
        unsigned int crc = xcrc32(data, 0, 0);
        REQUIRE(crc == 0);
    }
    ...
    SECTION("Known test vector") {
        ...
        unsigned int crc = xcrc32(data, 9, 0);
    ...
    SECTION("Accumulation") {
        ...
        unsigned int full_crc = xcrc32(full_data, 9, 0);
        
        // Calculate CRC32 for the data in two calls
        unsigned int crc1 = xcrc32(data1, 4, 0);
        unsigned int crc2 = xcrc32(data2, 5, crc1);
...
TEST_CASE("Integration: Roundtrip Conversion Test", "[Integration]") {
    ...
    // Use bin2srec to convert to SREC format (simulated call)
    {
        ...
        SrecFile sfile(srec_file, SrecFile::AddressSize::BITS32);
        ...
        SrecFile sfile2(tempfilename, SrecFile::AddressSize::BITS32);
    ...
    // Use sreccheck to verify the SREC file
    {
        ...
        while (std::getline(srecfile, line)) {
            if (line[0] == 'S') {
                ...
                calculated_sum = xcrc32(buff.data(), buff.size(), calculated_sum);
            }
        }
